...
headers = ['id','usercontragent_id','артикул товара ','бренд товара ','ip клиента']
...
for i in range(5000):
    ip_addresses = str([fake.ipv4_public()])
    clients_ip.append(ip_addresses)
    clients_ip = str(clients_ip + "::inet")
...
data = {
    ...
    "Бренд товара":brand,
    "IP клиента":clients_ip
}
df = pd.DataFrame(data)

for row_num, data in enumerate(zip(idd,usercontragent_id, arti_tov, brand, clients_ip), 2):
    for col_num, cell_data in enumerate(data, 1):
        col_letter = openpyxl.utils.get_column_letter(col_num)
        worksheet[f'{col_letter}{row_num}'] = cell_data

workbook.save('searchhistory.xlsx')